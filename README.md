...
算力设备1停用后，原2号连续确认中心广播缺席才接管为1号并发布新广播；聊天端在接管窗口内继续有限次重连，不会直接连仍编号为2的节点。轻量内存/状态每5秒轮询一次，完整设备清单只在成员变化时立即同步，另有30秒保底刷新。
...
算力编号按组网时的可用内存排一次（余量最大的是算力设备1），内存后来涨跌不再改号，界面上只显示「算力设备N / 聊天设备N」，不出现手机蓝牙名。聊天机按蓝牙名全网编号。回复末尾的「由算力设备N推理」是协调机改写 `done.source` 写上去的。
