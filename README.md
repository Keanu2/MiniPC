...
算力设备1停用后，存活算力按旧排名确认接管：至少15秒、三次成功完成的扫描均未发现旧中心后，由当前可见存活成员中排名最靠前的一台接管为1号并发布新广播；聊天端在接管窗口内继续有限次重连，不会直接连仍编号为2的节点。轻量内存/状态每2秒轮询一次，完整设备清单只在成员变化时立即同步，另有30秒保底刷新。最新问题、修复与验收步骤见 [2026-09-22 修复记录](docs/nearby-identity-and-hub-failover-fix-2026-09-22.md)。

消息走 `linkEnhance`，协议叫 `nearby-chat-v1`，帧大概 480 字节 Base64，编解码在 `protocol/ChatProtocol.ets`。

工作区新增多算力接入修复：中心为后加入节点统一分配编号，执行节点不自行猜测“2号”；成员变化在推理期间仍发送精简名单，按连接会话保证首次同步和失败重试。详情、验证结果和容量/真机边界见 [2026-09-23 多算力审查记录](docs/multi-compute-membership-review-2026-09-23.md)。
