...
1号停用后，2号在至少15秒且连续三次扫描都未见原中心时接管；短暂断链或仍能扫到原中心时不接管。内存/任务状态每2秒轻量更新，成员变化立即同步清单，完整清单另有30秒保底刷新。

内存、温度、CPU 和正在跑的路数每 2 秒轻量同步一次。多台算力按可用内存编号，余量最大的是算力设备1（算力中心）。聊天设备按蓝牙名全网编号。标题会显示自己是第几号。任务标题是「聊天设备N  由算力设备N推理」。也可以附图，同样只显示在任务卡上。点「停用近场」会关掉广播。
